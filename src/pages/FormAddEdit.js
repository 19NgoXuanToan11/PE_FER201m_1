...
const FormAddEdit = () => {
    ...
    const validateForm = () => {
        ...
        if (isNaN(rating) || parseInt(rating) < 1 || rating === '') {
            errors.rating_err = 'Rating must be a positive number and more than 0';
            isValid = false;
        }
        if (category.trim() === '') {
            ...
            <div className="form">
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    <div>
                        ...
                        <input type="number" name='rating' value={state.rating} onChange={handleInputChange} />
                        {errors.rating_err && <span className='error'>{errors.rating_err}</span>}
                    </div>
                    <div>
                        <label htmlFor="category">Category: </label>
                        <input type="text" name='category' value={state.category} onChange={handleInputChange} />
